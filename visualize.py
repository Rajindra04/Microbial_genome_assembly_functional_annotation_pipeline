...
def plot_cog_distribution(df, output_dir):
    ...
    plt.savefig(os.path.join(output_dir, 'cog_distribution.png'), bbox_inches='tight', dpi=150)
    plt.close()
    print("COG distribution plot saved with legend")
    
    return cog_df

def get_kegg_pathway_names(pathway_ids, cache_file='kegg_cache.csv', max_retries=3):
    ...
    if new_entries:
        try:
            ...
            print(f"Cached {len(new_entries)} new pathway names")
        except Exception as e:
...
def analyze_pathways(df, output_dir, use_ko_mapping=True):
    ...
    ko_cache_file = 'ko_pathway_cache.csv'
    ko_cache = {}
    ...
    if new_ko_entries:
        try:
            ...
            print(f"Cached {len(new_ko_entries)} new KO-to-pathway mappings")
        except Exception as e:
            print(f"Error writing to KO cache file: {e}")
    
    print(f"Fetching names for {len(pathway_genes)} pathways")
    pathway_names = get_kegg_pathway_names(pathway_genes.keys())
    ...
    plt.ylabel('Pathway')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'top_pathways.png'))
    plt.close()
    print("Top pathways plot saved")
    ...
    plt.xticks([])
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'pathway_heatmap.png'))
    plt.close()
    ...
    if len(top_pathway_ids) >= 3:
        ...
        plt.savefig(os.path.join(output_dir, 'pathway_overlap.png'))
        plt.close()
        print("Pathway overlap plot saved")
...
def main():
    ...
    output_dir = 'annotation_analysis_results'
