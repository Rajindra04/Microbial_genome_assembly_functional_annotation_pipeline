...
def taxonomic_classification(bins_dir, output_dir, resume=False, db_path=None):
    ...
    kraken_output = os.path.join(kraken_output_dir, "kraken2_output.txt")
    kraken_report = os.path.join(kraken_output_dir, "kraken2_report.txt")
    
    if resume and os.path.exists(kraken_output) and os.path.exists(kraken_report):
        logging.info("Kraken2 output files found, skipping taxonomic classification")
    ...
    command = (
        ...
        f"--output {kraken_output} --report {kraken_report} {bins_dir}/bin*.fa"
    )
    run_command(command, "Taxonomic classification with Kraken2")
    return kraken_output
...
def map_and_segregate_fastq(contigs, clean_fastq1, clean_fastq2, taxonomy_file, bins_dir, output_dir, resume=False):
    """Map FASTQ to binned contigs and segregate by organism."""
    logging.info(f"Using contigs file for mapping: {contigs}")
    # Parse Kraken2 output to map bins to taxonomic labels
    taxonomy = {}
    contig_to_taxa = {}
    with open(taxonomy_file, 'r') as f:
        for line in f:
            ...
            taxon_id = fields[2]
            if classification == 'C':
                contig_to_taxa[contig_id] = taxon_id.replace(' ', '_').replace(';', '_')
            else:
    ...
    if resume:
        ...
        for bin_id, organism in taxonomy.items():
            output_fastq1 = os.path.join(output_dir, f"{organism}_R1.fastq")
            output_fastq2 = os.path.join(output_dir, f"{organism}_R2.fastq")
            if not (os.path.exists(output_fastq1) and os.path.exists(output_fastq2)):
                all_segregated = False
                break
        if all_segregated:
            logging.info("All segregated FASTQ files found, skipping mapping and segregation")
            return
    ...
    for bin_file in Path(bins_dir).glob("bin*.fa"):
        ...
        if resume and os.path.exists(output_fastq1) and os.path.exists(output_fastq2):
            logging.info(f"Segregated FASTQ files for {organism} found, skipping")
            continue
        
        # Extract contig IDs from bin
        contig_ids = bin_to_contigs.get(bin_id, [])
        if not contig_ids:
            logging.warning(f"No contigs found for bin {bin_id}, skipping read extraction")
            continue
        
        # Create temporary file with contig IDs
        contig_list = os.path.join(output_dir, f"{bin_id}_contigs.txt")
        try:
            with open(contig_list, 'w') as f:
                for contig_id in contig_ids:
                    f.write(f"{contig_id}\t0\t9999999\n")
            logging.info(f"Created contig list for {bin_id}: {contig_list} with {len(contig_ids)} contigs")
        except Exception as e:
            logging.error(f"Failed to write contig list {contig_list}: {str(e)}")
            continue
        
        # Verify contig list file
        if not os.path.exists(contig_list) or os.path.getsize(contig_list) == 0:
            logging.warning(f"Contig list {contig_list} is empty or not created, skipping bin {bin_id}")
            if os.path.exists(contig_list):
                os.remove(contig_list)
            continue
        
        # Extract reads mapping to these contigs
        command = (
            f"samtools view -b {bam_file} -L {contig_list} | "
            f"samtools fastq -1 {output_fastq1} -2 {output_fastq2} -"
        )
        try:
            run_command(command, f"Extracting reads for {organism}")
        except Exception as e:
            logging.error(f"Failed to extract reads for bin {bin_id}: {str(e)}")
        finally:
            if os.path.exists(contig_list):
                os.remove(contig_list)  # Clean up
...
def main(input_fastq1, input_fastq2, output_dir, resume=False, db_path=None):
    ...
    try:
        ...
        taxonomy_file = taxonomic_classification(bins_dir, output_dir, resume, db_path)
        ...
        map_and_segregate_fastq(contigs, clean_fastq1, clean_fastq2, taxonomy_file, bins_dir, output_dir, resume)
